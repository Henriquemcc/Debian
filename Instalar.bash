...
function obter_opcao() {
  ...
  while ! [[ $_opcao_selecionada =~ $re ]] || [ "$_opcao_selecionada" -lt 0 ] || [ "$_opcao_selecionada" -gt 3 ]; do
    ...
    echo "0 - Sair"
    echo "1 - Instalar sistema HMCC-Server"
    echo "2 - Instalar sistema Henrique-Desktop"
    echo "3 - Instalar sistema Henrique-Laptop"
...
while [ "$opcao_selecionada" -ne 0 ]; do
  ...
  if [ "$opcao_selecionada" -eq 1 ]; then
    ...
    bash ./Instalar_Henrique-Desktop.bash
  elif [ "$opcao_selecionada" -eq 3 ]; then
    bash ./Instalar_Henrique-Laptop.bash
  fi
done
